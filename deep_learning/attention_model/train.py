"""
Trainer for the Attention Model (Kool et al. 2019).
...
class AttentionModelTrainer(BaseTrainer):
    """
    Trainer for the Attention Model.
    
    Uses REINFORCE with greedy rollout baseline:
    - For each batch, compute baseline by running greedy decoding
    - This provides lower variance than simple moving average
    """
    ...
    def __init__(self, model=None, device=None, lr=1e-4,
        ...
        # Baseline model (copy of main model for greedy rollout)
        self.baseline_model = None
        self.baseline_update_freq = 50  # Update baseline every N epochs
    ...
    def train_epoch(self, batch_size: int, num_nodes: int, epoch: int) -> float:
        ...
        if self.baseline_model is None:
            # First epoch: use moving average
            if epoch == 0:
        ...
        # Update moving average regardless
...
def train_model(num_epochs=100, batch_size=32, num_nodes=20,
                save_path="deep_learning/checkpoints/attention_model/vrp_model.pth",
                hidden_dim=128, num_heads=8, num_layers=3):
    """
...
if __name__ == "__main__":
    os.makedirs("deep_learning/checkpoints/attention_model", exist_ok=True)
    train_model(
        num_epochs=500, 
        batch_size=32, 
        num_nodes=30,
