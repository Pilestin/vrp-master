"""
Solver using the Attention Model (Kool et al. 2019).
"""
...
class AttentionModelSolver(BaseSolver):
    """
    VRP Solver using the Attention Model (Kool et al. 2019).
    
    Unlike the PointerNetworkSolver, this model handles capacity
    constraints during decoding, but we still apply post-hoc splitting
    for visualization consistency.
    """
    
    def __init__(self, graph, capacity=40,
                 model_path="deep_learning/checkpoints/attention_model/vrp_model.pth"):
    ...
    def load_model(self) -> bool:
        ...
        try:
            ...
            self.model.eval()
            print(f"Loaded Attention Model from {self.model_path}")
            return True
    ...
    def solve(self):
        """
        Solve VRP using the Attention Model.
